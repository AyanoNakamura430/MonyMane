...
カテゴリの削除可否判定は、対象IDと変動費実績の`categoryId`を厳密比較します。グループカテゴリは、最初に対象IDとカテゴリの`groupId`を厳密比較し、子カテゴリがなければ変動費実績の`categoryGroupId`を厳密比較します。子カテゴリと直接参照実績が両方ある場合は、子カテゴリありを優先します。

削除確認ダイアログは削除可否判定より先に表示します。削除可否の純粋関数はreasonだけを返し、日本語文言、state更新、配列の`filter`、編集状態解除、成功メッセージはUI側で処理します。
...
- 月次のカテゴリ予算とカテゴリ実績の比較
- 月次・年次のグループ別変動費集計と、選択グループのカテゴリ別内訳
- 変動費実績からのカテゴリ直接参照、子カテゴリ、変動費実績からのグループ直接参照に基づく削除制約

UI、React Hook統合、localStorageの全削除・一括保存、入力値整形は現在のunit test対象外です。

グループ・カテゴリ別内訳では、非空の`categoryGroupId`を優先し、欠落または空文字の場合は有効な`categoryId`から親グループを補完します。対象期間は月次では`month`の先頭7文字、年次では先頭4文字で判定します。表示対象は集計後の金額が正数の項目で、金額降順、同額時は対象実績で最初に登場した順です。割合は`Math.round(amount / total * 1000) / 10`で小数第1位相当に丸めます。

集計の純粋関数は意味的なID、金額、割合、区分、合計、選択グループの存在状態を返します。表示文言、色、Recharts用データ、選択操作、空状態の画面制御は`App.tsx`の責務です。
