...
- 現行アプリにはstrictなtypecheckがあるが、`vite.config.ts`と未参照コードは対象外
- lint、format、自動テスト、CIは未整備
...
初回unit testはNode環境で実行し、次の純粋なデータ処理を保護します。

- 同一年月の月間予算重複の正規化
- version 1相当のカテゴリ予算からversion 2の月間予算への移行計画
- backup version 1・2の解析と現在形式への復元
- 基本的な月次・年次の収入、固定費、変動費、予算集計

UI、React Hook統合、カテゴリ別・グループ別内訳、削除制約、localStorageの全削除・一括保存、入力値整形は初回unit testの対象外です。

カテゴリ予算からだけ参照されているカテゴリを削除できる挙動は既知の不整合です。期待仕様が未確定のためcharacterization testでは固定せず、削除制約の変更も行いません。
